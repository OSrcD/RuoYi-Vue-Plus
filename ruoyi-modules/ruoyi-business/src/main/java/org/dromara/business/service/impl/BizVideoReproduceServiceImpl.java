...
//        sb.append("以下为本单元图生视频提示词：\n\n");
    ...
    @Override
    public void generateAllVideos(Long taskId) {
    ...
    @Override
    public void washImage(Long frameId, String washMode, String customPrompt, List<String> refImages) {
    ...
    @Override
    public void washAllImages(Long taskId, String washMode, String customPrompt, List<String> refImages) {
        List<BizVideoReproduceFrame> frames = getFrames(taskId);
        for (BizVideoReproduceFrame frame : frames) {
            washImage(frame.getFrameId(), washMode, customPrompt, refImages);
        }
    ...
    @Override
    public void generateVideo(Long frameId) {
        BizVideoReproduceFrame frame = frameMapper.selectById(frameId);
        if (frame == null) {
            throw new ServiceException("截帧不存在");
        }
